...
type ConcordancePayload = {
  schema: "oretsuba-public-concordance/1";
  version: string;
  totalLines: number;
  fields: [
    "ref",
...
export function ScriptBrowser() {
    ...
    fetch("../script-data/index.json")
      ...
      .then((data: ScriptIndex) => {
        ...
        const comparisonEnabled =
          Boolean(data.comparison) && url.searchParams.get("compare") === "todokanai";
        setShowTodokanai(comparisonEnabled);
        setShowTodokanaiErrors(
          comparisonEnabled &&
            url.searchParams.get("errors") === "todokanai",
        );
        const legacyPrologue = url.searchParams.get("script") === "s00_00" && url.searchParams.get("route") === "section-00";
        const requestedRouteId = legacyPrologue ? "prologue" : url.searchParams.get("route") === "section-03c" ? "section-03" : url.searchParams.get("route");
        ...
        const hashRef = decodeURIComponent(url.hash.slice(1)).replace(/^oretsuba:section-00:s00_00:/, "oretsuba:prologue:s00_00:").replace(/^oretsuba:section-03c:/, "oretsuba:section-03:");
        const hashParts = hashRef.split(":");
        if (hashParts.length === 4 && hashParts[0] === "oretsuba") {
          const hashRouteId = hashParts[1];
          ...
          const legacyCorpusRoute = url.searchParams.get("section") ?? url.searchParams.get("chapter");
          const requestedCorpusRoute = legacyCorpusRoute === "section-03c" ? "section-03" : legacyCorpusRoute;
          if (
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      if (searchScope === "corpus") {
        ...
        const normalizedHash = decodeURIComponent(url.hash.slice(1)).replace(/^oretsuba:section-03c:/, "oretsuba:section-03:");
        const hashParts = normalizedHash.split(":");
        const hashRouteId =
          hashParts[0] === "oretsuba"
            ? hashParts[1]
            : hashParts[0] === "wa2mas"
            ? "special"
            : hashParts[1] === "ic"
  ...
  useEffect(() => {
    ...
    fetch(`../script-data/${filename}`, { signal: controller.signal })
      ...
      .then((data: ConcordancePayload) => {
        if (
          data.schema !== "oretsuba-public-concordance/1" ||
          data.totalLines !== index.totalLines
        ) {
  ...
  return (
    <section className="reader-shell shell compact">
      ...
      <p className="reader-note">English script · v1.1.6.</p>
    </section>
  );
}
