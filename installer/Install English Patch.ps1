...
try {
 ...
 try {
  ...
  $installedFont = Join-Path $fontDir 'MAO-BIZUDGothic-Regular.ttf'
  Copy-Item -LiteralPath $fontPath -Destination $installedFont -Force
  $key = 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Fonts'
  New-Item -Path $key -Force | Out-Null
  New-ItemProperty -Path $key -Name 'BIZ UDGothic (TrueType)' -Value $installedFont -PropertyType String -Force | Out-Null
 ...
 Write-Host 'English v1.1.0 installed. Originals are in MAO-original-backup. Start ORE_TUBA.EXE.' -ForegroundColor Green
